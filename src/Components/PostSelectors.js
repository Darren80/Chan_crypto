    ...
    if (!this.processing) {
      ...
      if ("connection" in navigator) {
        if (navigator.connection.saveData === true) {
          ...
          //compressed: `https://images.cryptostar.ga/file/lon1-static/images_lossless/${tim}${ext}`,
        } else {
          imageUrl = `https://images.cryptostar.ga/file/lon1-static/images_compressed_webp/${tim}${ext}`
        }
      }
    }

    fetch(`${imageUrl.lossless}`).then(response => {
